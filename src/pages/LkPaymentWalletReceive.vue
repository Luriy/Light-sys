...
export default {
  ...
  computed: {
    linkBlockchain() {
      switch (this.$route.params.currency.toUpperCase()) {
        case 'BTC': return `https://www.blockchain.com/ru/btc/address/${this.$route.params.address}`;
        case 'ETH': return `https://etherscan.io/address/${this.$route.params.address}`;
        case 'LTC': return `https://bchain.info/LTC/tx/${this.route.params.address}`;
  ...
  methods: {
    sendEmail() {
      window.open(`mailto:?subject=LightNet ${this.currencyName} Address&body=My LightNet ${this.currencyName} address is: `+this.$route.params.address);
    },
    printWallet() {
