...
export default {
	...
	mutations: {
		SET_DIRECTION_STATUS: (state, { index, directionStatus }) =>
			(state.exchangeData[index].directionStatus = directionStatus),
	...
	actions: {
		...
		GET_FIAT_EXCHANGE: ({ commit }) => {
			...
			Promise.all([fiatPsids, allWallets, cardsList]).then(function(values) {
				...
				const resultWallets = currencyKeys
					.reduce(
						(acc, currencyKey) => [
							...acc,
				...
				const resultCards = Object.keys(cards)
					.map((card) => {
						const { name, reserve, valute } = fiat[+cards[card].Psid];
						return {
							fullName: name,
							icon:
								fiatList[cards[card].Psid] && fiatList[cards[card].Psid].icon.big
							...
							...(!cards[card].StatusExchange && {
								directions: Object.values(cards[card].ExchangeVariable),
							}),
						};
					})
